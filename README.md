...
一个面向 Codex 的 B 站课程处理 Skill：从公开的 `bilibili.com` 或 `b23.tv` 链接开始，生成压缩音频、本地逐字稿、忠实详细的分集笔记、可点击目录和独立的精炼合集。
...
Skill 会先检查课程结构和疑似重复合集，再下载、转换、转写、生成详细笔记与导航，最后执行完整性检查。

也可以单独运行脚本：

```powershell
python process-bilibili-course/scripts/bilibili_pipeline.py inspect `
  --url 'https://www.bilibili.com/video/BVxxxxxxxxxx' `
  --workspace 'D:\Course Notes'

python process-bilibili-course/scripts/bilibili_pipeline.py run `
  --url 'https://www.bilibili.com/video/BVxxxxxxxxxx' `
  --workspace 'D:\Course Notes'
```
...
- 如访问必须使用浏览器 Cookie，Skill 会先请求用户明确许可，并禁止把 Cookie 写入笔记或日志。
- 请遵守 B 站服务条款、视频作者许可和所在地法律；MIT 许可证仅覆盖本仓库的代码与文档，不覆盖下载内容。
...
This Codex Skill turns a public Bilibili video or multi-part course into compressed audio, local transcripts, faithful detailed episode notes, a linked course index, and a separate concise review collection.
...
3. Restart Codex.
4. Provide a Bilibili URL and ask Codex to use `$process-bilibili-course` on every unique episode.
